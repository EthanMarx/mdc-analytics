...
PIPELINE_TO_SKYMAP = {
    ...
    "preferred": "bayestar.multiorder.fits",
...
def query_gevents(
    ...
    if pipeline == "preferred":
        ...
        response = []
        for batch in batched(pipeline_events.preferred_event.values, 1000):
...
def _process_skymap(
    ...
    # first, crossmatch on using uploaded skymap
    ...
    results = {}
    results[skymap_instruments] = result

    # if pipeline is not aframe or cwb,
    # see if we can run bayestar on different
    # requested ifo configurations
    if pipeline not in ("aframe", "cwb"):
        # read in SNR timeseries
        ...
        # read in instruments in snr time series
        ...
        assert instruments == skymap_instruments

        # for each config,
        for ifo_config in bayestar_ifo_configs:
            if ifo_config == skymap_instruments:
                continue

            disable_detectors = instruments - ifo_config
            if disable_detectors and ifo_config <= instruments:
...
def process_skymaps(
    ...
    futures = parallelize(func, events, max_workers=15)
    ifo_config_strs = [
    ...
    for ifo_config in results.keys():
        ...
        for key in CROSSMATCH_KEYS:
            events[f"{pipeline}_{key}_{ifo_config_str}"] = [
                getattr(result, key) if result else None
...
def process_posteriors(events: pd.DataFrame, gdb_server: str):
    ...
    for future in tqdm(
        ...
        try:
            ...
            logging.info(
                f"Failed to query posterior for {events.aframe_graceid.values[idx]}"
            )
            results[idx] = None
...
def _process_coinc(
    ...
    doc = load_fileobj(coinc_data, contenthandler=events.ligolw.ContentHandler)
...
def process_coincs(events: pd.DataFrame, gdb_server: str, pipeline: str):
...
def process_embrights(
    ...
    for future in tqdm(
        ...
        try:
            ...
            results[idx] = None
            logging.info(f"Failed to fetch em bright for {gids[idx]}")

    for key in EM_BRIGHT_KEYS:
        output = []
        for result in results:
            output.append(
                result[key] if result is not None and key in result else None
            )
        events[f"{pipeline}_" + key] = output
